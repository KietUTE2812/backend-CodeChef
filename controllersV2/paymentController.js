...
const getPayments = asyncHandler(async (req, res, next) => {
    ...
    if (filters.to || filters.from) {
        query.createdAt = {
            ...(filters.from && { $gte: new Date(parseDate(filters.from)) }),
            ...(filters.to && { $lte: new Date(parseDate(filters.to)) })
        };
        delete query.from;
        delete query.to;
    }

    const payments = await Payment.find(query).limit(limit).skip(limit * (page - 1)).sort({ createdAt: -1 })
        .populate('userId', 'profile').populate('courseId', 'title');
    ...
    res.status(200).json({ success: true, limit, page, count: payments.length,total: {totalSuccess, totalPending, totalFailed}, data: {
            payments,
        } });
});
...
const updatePayment = asyncHandler(async (req, res, next) => {
    ...
    if (req.user.role!== 'admin') {
        return next(new ErrorResponse(`User ${req.user._id} is not authorized to update payment ${req.params._id}`, 401));
    }
...
const deletePayment = asyncHandler(async (req, res, next) => {
    ...
    // Make sure user is payment owner
    if (req.user.role!== 'admin') {
        return next(new ErrorResponse(`User ${req.user._id} is not authorized to delete payment ${req.params._id}`, 401));
    }

    await Payment.findByIdAndUpdate(req.params.id, { isRemoved: true });

    res.status(200).json({ success: true, data: {} });
});
export default {getPayments, getPaymentById, createPayment, updatePayment, deletePayment, vnpayReturn, vnPayIPN};
